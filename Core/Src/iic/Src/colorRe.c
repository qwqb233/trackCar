...
*******************************************************************************/
/*void TCS34725_I2C_Write(I2C_TypeDef* I2Cx,u8 slaveAddress, u8* dataBuffer,u8 bytesNumber, u8 stopBit)
{
    u8 i = 0;
    *//*TCS34725_I2C_Start();
    TCS34725_I2C_Send_Byte((slaveAddress << 1) | 0x00);	   //发送从机地址写命令
    TCS34725_I2C_Wait_ACK();*//*
    I2C_master_transmit(I2Cx,TCS34725_ADDRESS, (u8*)&slaveAddress, 1);
...
void TCS34725_Write(I2C_HandleTypeDef *I2Cx,u8 subAddr, u8* dataBuffer, u8 bytesNumber)
{
    ...
    I2C_master_transmit(I2Cx,TCS34725_ADDRESS, sendBuffer, bytesNumber + 1);
}
/*******************************************************************************
...
void TCS34725_Read(I2C_HandleTypeDef *I2Cx,u8 subAddr, u8* dataBuffer, u8 bytesNumber)
{
    ...
    I2C_master_transmit(I2Cx,TCS34725_ADDRESS, &subAddr, 1);
    I2C_master_receive(I2Cx,TCS34725_ADDRESS, dataBuffer, bytesNumber);
}
/*******************************************************************************
...
{
    TCS34725_Write(I2Cx,TCS34725_ATIME, &time, 1);
}
/*******************************************************************************
 * @brief TCS34725设置增益
 *
...
{
    TCS34725_Write(I2Cx,TCS34725_CONTROL, &gain, 1);
}
/*******************************************************************************
 * @brief TCS34725使能
 *
...
u8 TCS34725_Init(I2C_HandleTypeDef *I2Cx)
{
    ...
    TCS34725_Read(I2Cx,TCS34725_ID, &id, 1);  //TCS34725 的 ID 是 0x44 可以根据这个来判断是否成功连接,0x4D是TCS34727;
    if(id==0x4D | id==0x44)
    {
        TCS34725_SetIntegrationTime(I2Cx,TCS34725_INTEGRATIONTIME_50MS);
        TCS34725_SetGain(I2Cx,TCS34725_GAIN_1X);
        TCS34725_Enable(I2Cx);
        return 1;
    }
    return 0;
}
...
u8 TCS34725_GetRawData(I2C_HandleTypeDef *I2Cx,COLOR_RGBC *rgbc)
{
    ...
    {
        rgbc->c = TCS34725_GetChannelData(I2Cx,TCS34725_CDATAL);
        rgbc->r = TCS34725_GetChannelData(I2Cx,TCS34725_RDATAL);
        rgbc->g = TCS34725_GetChannelData(I2Cx,TCS34725_GDATAL);
        rgbc->b = TCS34725_GetChannelData(I2Cx,TCS34725_BDATAL);
        return 1;
    }
    return 0;
}
...
{
    TCS34725_GetRawData(I2Cx,&rgb);  //读两次，实际测试时发现读到的颜色总是上一次的颜色
    RGBtoHSL(&rgb,&hsl);
}
